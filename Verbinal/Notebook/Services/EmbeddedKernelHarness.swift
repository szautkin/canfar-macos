...
/// Embedded Python kernel harness script.
/// This ensures the harness is always available even if the bundle resource is missing.
enum EmbeddedKernelHarness {
    static let source = #"""
...
def send_status(state): send({"type": "status", "state": state})
...
def _handle_magic(line):
    ...
    if line.startswith("%pip ") or line.startswith("!pip "):
        ...
        try:
            ...
            outputs = []
            if result.stdout: outputs.append({"type": "stream", "name": "stdout", "text": result.stdout})
            if result.stderr: outputs.append({"type": "stream", "name": "stderr", "text": result.stderr})
            return outputs
    ...
    if line.startswith("!"):
        ...
        try:
            ...
            collected = []
            for out_line in proc.stdout: collected.append(out_line)
            proc.wait(timeout=120)
            text = "".join(collected)
            outputs = []
            if text: outputs.append({"type": "stream", "name": "stdout", "text": text})
            return outputs
        except Exception as e:
            return [{"type": "error", "ename": type(e).__name__, "evalue": str(e), "traceback": [str(e)]}]
...
def execute_code(code, exec_count):
    ...
    try:
        ...
        success = False
        tb = e.__traceback__
        while tb is not None and "kernel_harness" in (tb.tb_frame.f_code.co_filename or ""): tb = tb.tb_next
        if tb is None: tb = e.__traceback__
    ...
    if stdout_text: outputs.append({"type": "stream", "name": "stdout", "text": stdout_text})
    stderr_text = captured_err.getvalue()
    if stderr_text: outputs.append({"type": "stream", "name": "stderr", "text": stderr_text})
    try:
        import matplotlib.pyplot as plt
        figs = [plt.figure(i) for i in plt.get_fignums()]
        for fig in figs:
            display = _capture_display_data(fig)
            if display: outputs.append({"type": "display_data", "data": display})
        plt.close("all")
    except ImportError: pass
    if result is not None and success:
        display = _capture_display_data(result)
        if display: outputs.append({"type": "display_data", "data": display})
        else: outputs.append({"type": "execute_result", "data": {"text/plain": repr(result)}, "exec_count": exec_count})
    for out in outputs: send(out)
...
def main():
    ...
    for line in sys.stdin:
        ...
        if msg_type == "quit": break
        elif msg_type == "execute":
            code = msg.get("code", "")
            exec_count = msg.get("exec_count", 0)
            if code.strip(): execute_code(code, exec_count)
            else:
                send({"type": "execute_reply", "exec_count": exec_count, "success": True})
                send_boundary()
