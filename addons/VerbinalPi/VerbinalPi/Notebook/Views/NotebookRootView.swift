...
struct NotebookRootView: View {
    /// The addon-shell owns the tab host so it can open files handed in via
    /// `AddonBeacon` activations. Pass it down as a parameter rather than
    /// owning it with `@State` here.
